...
ORIGIN_LAT = 27.375608
ORIGIN_LON = -82.452475
GRID_SIZE_M = 100.0
PX_PER_M = 8.0
W_PX = H_PX = int(GRID_SIZE_M * PX_PER_M)
R_EARTH = 6371000.0 

# Origin anchored at 90% Y to provide 90m forward view (West is UP)
CENTER_X_PX = W_PX / 2
...
class MapItem:
    """Shared logic for path waypoints and surveyed objects."""
    def setup_base(self, label_text):
        ...
        self.start_pos = self.pos() # Record the 'Birth' position for delta tracking
    ...
    def get_displacement_m(self):
        """Calculates distance from creation point in meters."""
...
class MissionElement(QGraphicsRectItem, MapItem):
    def __init__(self, x, y, label_id, on_move):
        super().__init__(-6, -6, 12, 12)
        self.setBrush(QBrush(Qt.GlobalColor.green))
        self.setPos(x, y)
        self.on_move = on_move
        self.setup_base(f"O{label_id}")
        self.update_coords()
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("RoboBoat 2026 | Surveyor & Precision Editor")
        ...
        btn_proj = QPushButton("🚀 Project Object")
        btn_proj.clicked.connect(self.add_surveyed_element)
        s_lay.addWidget(QLabel("Obs:")); s_lay.addWidget(self.in_lat); s_lay.addWidget(self.in_lon)
        s_lay.addWidget(self.in_brg); s_lay.addWidget(self.in_dist); s_lay.addWidget(btn_proj)

        # --- Edit Panel with Move Tracker ---
        edit_w = QWidget(); e_vlay = QVBoxLayout(edit_w)
        e_hlay1 = QHBoxLayout()
        self.lbl_move_delta = QLabel("Move Delta: 0.00m")
        self.lbl_move_delta.setStyleSheet("color: blue; font-weight: bold; font-size: 14px;")
        
        self.nudge_up = QLineEdit("0.0"); self.nudge_up.setFixedWidth(50)
        self.nudge_right = QLineEdit("0.0"); self.nudge_right.setFixedWidth(50)
        btn_nudge = QPushButton("Nudge")
        btn_nudge.clicked.connect(self.nudge_selected_item)
        
        e_hlay1.addWidget(self.lbl_move_delta)
        e_hlay1.addSpacing(20)
        e_hlay1.addWidget(QLabel("Up (W):")); e_hlay1.addWidget(self.nudge_up)
        e_hlay1.addWidget(QLabel("Right (N):")); e_hlay1.addWidget(self.nudge_right)
        e_hlay1.addWidget(btn_nudge)
        e_hlay1.addStretch()
        btn_save = QPushButton("💾 Save JSON")
        btn_save.clicked.connect(self.save_mission)
        e_hlay1.addWidget(btn_save)
        ...
        right_panel.addWidget(QLabel("<b>COORDINATE LIST</b>"))
    ...
    def switch_mode(self, index):
        ...
        if not is_edit: self.lbl_move_delta.setText("Move Delta: Locked")
    ...
    def handle_selection_event(self):
        """Syncs table and updates the Move Delta display."""
        selected = self.scene.selectedItems()
        if not selected or self.panel_stack.currentIndex() == 0:
            self.lbl_move_delta.setText("Move Delta: --")
            return
        
        item = selected[0]
        if isinstance(item, MapItem):
            delta = item.get_displacement_m()
            self.lbl_move_delta.setText(f"Move Delta: {delta:.2f}m")
            # Sync Table
            id_text = item.label.toPlainText()
            for row in range(self.table.rowCount()):
                if self.table.item(row, 0).text() == id_text:
    ...
    def nudge_selected_item(self):
        ...
        try:
            dy_px = -float(self.nudge_up.text()) * PX_PER_M
            dx_px = float(self.nudge_right.text()) * PX_PER_M
            for item in selected:
                item.setPos(item.pos().x() + dx_px, item.pos().y() + dy_px)
            self.refresh_table_data()
            self.handle_selection_event() # Update delta label
        except ValueError: pass

    def add_surveyed_element(self):
        try:
            ...
            obj = MissionElement(px, py, len(self.elements)+1, self.refresh_table_data)
            self.scene.addItem(obj); self.elements.append(obj)
            self.refresh_table_data()
        except: pass

    def handle_map_double_click(self, event):
        if self.panel_stack.currentIndex() == 1:
            pos = self.view.mapToScene(event.pos())
            wp = Waypoint(pos.x(), pos.y(), len(self.waypoints)+1, self.refresh_table_data)
    ...
    def refresh_table_data(self, item=None):
        ...
        if item: self.handle_selection_event()
    ...
    def add_table_row(self, id_str, type_str, lat, lon):
        ...
        self.table.setItem(row, 2, QTableWidgetItem(f"{lat:.7f}"))
        self.table.setItem(row, 3, QTableWidgetItem(f"{lon:.7f}"))

    def save_mission(self):
        path, _ = QFileDialog.getSaveFileName(self, "Save", "mission_plan.json", "JSON (*.json)")
        if path:
            data = {
